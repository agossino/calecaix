                            ...
                            <tbody>
                                @foreach ($iscritti as $iscritto)
                                    @if ($user->role == 'amministratore' || $user->role == 'accompagnatore_editor')
                                        <tr>
                                            ...
                                            <td>
                                                <select id="stato_iscrizione" name="stato_iscrizione[{{ $iscritto->id }}]" class="form-select" onchange="this.form.submit()">
                                                    <option value="{{ $iscritto->stato_iscrizione }}">
                                                        {{ $stato_iscrizione->firstWhere('id', $iscritto->stato_iscrizione)->nome ?? '' }}
                                                    </option>
                                                    @foreach ($stato_iscrizione as $attiv)
                                                        <option value="{{ $attiv->id }}"> {{ $attiv->nome }} </option>
                                                    @endforeach
                                                </select>
                                            </td>
                                ...
                                @elseif($user->role == 'accompagnatore')
                                    <tr>
                                        ...
                                        <td>{{ $iscritto->socio_cai }}</td>
                                        @if ($iscritto->sezione != null && $iscritto->sezione != 'Selezione')
                                             {{$cai_sezioni->firstWhere('id', $iscritto->sezione)->nome ?? '' }}</td>
                                        @else
                                            <td></td>
                                        @endif
                                        <td>{{ $tipo_iscrizione[$iscritto->iscrizione_tipo]->nome }}</td>
